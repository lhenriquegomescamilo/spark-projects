...
object ReporterPipeline {
  ...
  def downloadDay(
    ...
    val columns =
      """device_id, id_partner, event_type, device_type, first_party, third_party, country, time"""
        .replace("\n", "")
        .replace(" ", "")
        .split(",")
        .toList
    ...
    // Window to select the last occurrence of a user for a given partner
    val w = Window
      .partitionBy(col("device_id"), col("id_partner"))
    ...
    val filtered =
      finalDF
        ...
        .where(col("rn") === 1)
        .drop("event_type", "time", "datetime", "rn")
    ...
    val outputPath = "/datascience/data_reporter2" + (if (parallel > 0)
                                                        "_%s".format(
                                                          parallel
                                                        )
                                                      else "")
  ...
  def main(args: Array[String]) {
    ...
    downloadDay(spark, from, parallel)
  }
}
