...
//This function is used with POIs
  def userAggregate(
    ...
    val data = spark.read
      ...
      .option("sep", "\t")
      .option("header", "true")
      .load("/datascience/geo/geodevicer_bot/outputs/%s/raw_output".format(value_dictionary("poi_output_file")))
...
//this function sis used to create the map with POIs
  ...
  def POIAggregate(
    ...
    val data = spark.read
      ...
      .option("sep", "\t")
      .load(
        "/datascience/geo/geo_devicer_bot/outputs/%s/aggregated".format(value_dictionary("poi_output_file"))
      )
    ...
    data
      ...
      .mode(SaveMode.Overwrite)
      .save(
        "/datascience/geo/geodevicer_bot/outputs/%s/map_data/"
...
def POIAggregate_w_xd(
  ...
  val xd_result = spark.read      
                  ...
                  .option("sep", "\t")
                  .load(
        "/datascience/geo/geodevicer_bot/%s/crossdeviced/"
          .format(value_dictionary("poi_output_file")))
  ...
  map_data.join(countByCode,Seq(value_dictionary("poi_column_name")))
    ...
    .option("sep", "\t")
    .mode(SaveMode.Overwrite)
    .save( "/datascience/geo/geodevicer_bot/outputs/%s/map_w_xd/"
          .format(value_dictionary("poi_output_file"))
      )            
...
def userAggregateFromPolygon(
    ...
    // Path where we will store the results.
    val output_path_anlytics = "/datascience/geo/geodevicer_bot/%s/aggregated"
      .format(value_dictionary("poi_output_file"))
 ...
 def getDataPipeline(
  ...
  //add segments
    def get_segments  (
      ...
      value_dictionary: Map[String, String]
        ...
        .option("sep", "\t")
        .load(
        "/datascience/geo/geodevicer_bot/outputs/%s/xd"
          .format(value_dictionary("poi_output_file"))
  ...
  //add segments
    def get_segments_from_triplets(
      ...
      val output_path_horrible_users = "/datascience/geo/geodevicer_bot/outputs/%s/users_data"
                                                            .format(value_dictionary("poi_output_file"))
  ...
  //This function takes as input a number. this number is substracted from the current date
    ...
    //val fidelity_user = user_attributions.filter("fidelity_user == 1").select("device_type","device_id",value_dictionary("audience_column_name"))
      ...
      .option("sep", "\t")
      .mode(SaveMode.Overwrite)
      .save("/datascience/geo/geodevicer_bot/outputs/%s/att_date-%s"
...
def user_aggregate_for_moving_transport(
    ...
    val exploded_by_transport_id =  filter_by_distance
      ...
      .withColumn(value_dictionary("poi_column_name"),col("transport_id")) 
               ...
               // Path where we will store the results.
    val output_path_anlytics = "/datascience/geo/geodevicer_bot/outputs/%s/aggregated"
      .format(value_dictionary("poi_output_file"))
