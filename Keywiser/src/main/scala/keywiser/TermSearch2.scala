...
/**
  ...
  def MainProcess(
    ...
    val df_selected1 = getSelectedKeywordsOLD(spark,12,26)
    val df_selected2 = getSelectedKeywords(spark,26,0)
    ...
    val data_urls = getDataUrls(spark,"AR",26,0)
    ...
    df.groupBy("device_id","search_terms").agg(approx_count_distinct(col("url"), 0.02).as("url_count"))
      .write.format("csv")
      .option("header",true)
      .mode(SaveMode.Overwrite)
      .save(path2)
    ...
    db.groupBy("device_id").agg(sum(col("url_count")).as("url_count_total"))
      .select("device_id","url_count_total")
      .write.format("csv")
      .option("header",true)
      .mode(SaveMode.Overwrite)
      .save(path3)
    ...
    val df_joint = db.join(count_total,Seq("device_id"))
    .filter("search_terms==1")
    .withColumn("ratio",col("url_count")/col("url_count_total"))
    .select("device_id","url_count","url_count_total","ratio")
