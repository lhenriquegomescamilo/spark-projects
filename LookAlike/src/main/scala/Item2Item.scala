...
object Item2Item {
  ...
  */
  def runExpand(spark: SparkSession,
    ...
    // We use startapp segments in BR to imporve volumes
    if(isOnDemand && country == "BR"){
      val startappSegments = getLatamStartappSegments()
      baseFeatureSegments ++= startappSegments.toSet.diff(baseFeatureSegments.toSet).toList
    }
