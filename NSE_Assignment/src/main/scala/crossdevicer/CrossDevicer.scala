...
object CrossDevicer {
  ...
  def cross_device(
    ...
    val db_data = spark.read
      ...
      .join(        
        audience    
        .select("device_id","device_type","NSE","GEOID","audience","frequency","latitude","longitude")                 
        .distinct(),        
        Seq("device_id"),
            "right_outer")      
      ...
      .select("device_type","device_id","audience","frequency","NSE","GEOID","latitude","longitude") 
