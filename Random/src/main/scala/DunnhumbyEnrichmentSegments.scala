...
object DunnhumbyEnrichmentSegments {
...
**/
  ...
  **/
  ...
  def getEnrichment(
    ...
    val all_segments = (560 to 576)
      .map("array_contains(all_segments, %s)".format(_))
      .mkString(" OR ")

    // Query with the list of campaign segments
    val segments = cmp_segments
      .map("array_contains(segments, %s)".format(_))
      .mkString(" OR ")
    ...
    // UDF to get the campaign id based on the cmp segment
    val getCMPid = udf(
      (event_segments: Seq[Int]) =>
        cmp_segments.filter(s => event_segments.contains(s))(0)
    )
    ...
    val joint = data
      .filter(query)
      .withColumn("campaign_id", getCMPid(col("segments")))
