...
object GetDataXPCL {
  ...
  /**
    * GET UA DATA FOR A GIVEN AUDIENCE
    */
  def getUAData(spark: SparkSession, expansion: Boolean) = {
    val user_agents =
      get_data_user_agents(spark, 60, 1, "CL")

    val data = user_agents
    ...
    if (expansion) {
      data.write
        .format("parquet")
        .mode("overwrite")
        .save("/datascience/custom/cl_users_ua_xp")
    } else {
      ...
      data
        .join(audience, Seq("device_id"), "inner")
        .write
        .format("parquet")
        .mode("overwrite")
        .save("/datascience/custom/cl_users_ua_train")
    }
  }
  ...
  /**
    ...
    */
  def getURLData(spark: SparkSession, expansion: Boolean) = {
    val urls =
      get_data_urls(spark, 60, 1, "CL").select(
        "device_id",
        "url"
      )
    ...
    if (expansion) {
      ...
      urls
        ...
        .format("parquet")
        .mode("overwrite")
        .save("/datascience/custom/cl_users_xp")
    } else {
      val audience = spark.read
        .format("csv")
        .option("sep", "\t")
      ...
      urls
        .join(audience, Seq("device_id"), "inner")
        .select("device_id", "url", "ids")
        .dropDuplicates("device_id", "url")
        .write
        .format("parquet")
        .mode("overwrite")
        .save("/datascience/custom/cl_users_urls_train")
    }
  ...
  def get_joint(spark: SparkSession) = {
    val ua_df =
      spark.read.format("parquet").load("/datascience/custom/cl_users_ua_xp")
    val urls =
      spark.read.format("parquet").load("/datascience/custom/cl_users_urls_xp")
  ...
  def getDataSegments(spark: SparkSession, expansion: Boolean) = {
    ...
    val data_segments = getDataTriplets(spark, country = "CL", nDays = 60)
    ...
    if (expansion) {
      ...
      audience
        ...
        .option("sep", "\t")
        .mode("overwrite")
        .save("/datascience/custom/cl_users_segments_train")
    }
