...
object Retroactivations {
  ...
  def streamCSVs(
    ...
    val partners = List(167,280,289,317,443,471,486,517,637,643,657,661,683,693,714,761,919,1111,1155,1213)
      ...
      if (partition == "country")
        ...
        finalDF
          .filter(
            length(col("device_id")) > 0 && col("event_type")
              .isin(event_types: _*) && col("id_partner")
              .cast(IntegerType).isin(partners: _*)
          )
