...
object GenerateGTJson{
  ...
  def generate_json_genre_ar(spark:SparkSession, conf:Configuration, current_month: String){
    ...
    var os = fs.create(new Path("/datascience/devicer/to_process/AR_genero_%s.json".format(current_month)))
  ...
  def generate_json_age_ar(spark:SparkSession, conf:Configuration, current_month: String){
    ...
    var fs = FileSystem.get(conf)
    var os = fs.create(new Path("/datascience/devicer/to_process/AR_edad_%s.json".format(current_month)))
  ...
  def generate_json_genre_mx(spark:SparkSession, conf:Configuration, current_month: String){
    ...
    val os = fs.create(new Path("/datascience/devicer/to_process/MX_genero_%s.json".format(current_month)))

    val content = """{"xd": 0, "partnerId": "", "query": "(country = 'MX' and array_contains(all_segments, 69228))", "ndays": 30, "queue": "datascience", "pipeline": 0, "segmentId": 2, "since": 1, "priority": 14, "common": "country = 'MX'", "as_view": 119, "push": 0, "xdFilter": "index_type = 'coo'"}
  ...
  def generate_json_age_mx(spark:SparkSession, conf:Configuration, current_month: String){
    ...
    val fs = FileSystem.get(conf)
    val os = fs.create(new Path("/datascience/devicer/to_process/MX_edad_%s.json".format(current_month)))

    val content = """{"xd": 0, "partnerId": "", "query": "(country = 'MX' and array_contains(all_segments, 106399))", "ndays": 30, "queue": "datascience", "pipeline": 0, "segmentId": 8, "since": 1, "priority": 14, "common": "country = 'MX'", "as_view": 119, "push": 0, "xdFilter": "index_type = 'coo'"}
