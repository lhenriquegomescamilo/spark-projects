      ...
      .map(
        x =>
          spark.read
            ...
            .parquet(x)
            .filter("(country = 'AR' or country = 'MX' or country = 'BR') and url is not null and event_type in ('batch','pv','data')") // We only get AR and MX users because we only have GA data for those countries
