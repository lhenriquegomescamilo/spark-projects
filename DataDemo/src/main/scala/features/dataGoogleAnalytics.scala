...
object DataGoogleAnalytics {
  ...
  /**
    ...
    *
    */
  def get_data_google_analytics(spark: SparkSession, ndays: Int, filename:String) {
  ...
  /**
    ...
    *
    */
    def get_data_google_analytics_path(spark: SparkSession, ndays: Int, filename:String) {
      /// Configuraciones de spark
      val sc = spark.sparkContext
      val conf = sc.hadoopConfiguration
      val fs = org.apache.hadoop.fs.FileSystem.get(conf)
      ...
      joint.write
        ...
        .partitionBy("country")
        .mode(SaveMode.Overwrite)
        .save("/datascience/data_demo/google_analytics_path")
    }
  ...
  def main(args: Array[String]) {
    ...
    get_data_google_analytics(spark, ndays, filename_domain)
    get_data_google_analytics_path(spark, ndays, filename_path)
  }
}
