...
object asignarNSE {
  ...
  def getPolygons(spark: SparkSession) = {
				...
				val geojson_path_formated =      "/datascience/geo/polygons/MX/NSE/geojson/MX_ageb_NSE_formatted"
				...
				val ageb_nse = names.join(polygonDf,Seq("rowId1")).drop("rowId1").withColumn("CVEGEO",col("properties.CVEGEO")).withColumn("NSE",col("properties.NSE_segment"))
