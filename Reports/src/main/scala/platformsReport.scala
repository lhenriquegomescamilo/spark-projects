...
object platformsReport {
  ...
  def getStringRepresentation =
    udf((n: Int) => (toBinary(n).reverse.padTo(5, '0')).map(_.toString))

  /** Map representation to combinations of platforms */
  def getPlatforms =
    udf(
      (j: IndexedSeq[String]) =>
        ((IndexedSeq("d2", "d10", "d11", "d13", "d14") zip j).toMap
          .filter({ case (k, v) => v == "1" }))
          .keys
          .toList
    )
  ...
  def udfMapCountries =
    udf((segments: Seq[Int]) => (segments flatMap (seg_country_map get)))

  def udfFilterCountries =
    udf(
      (segments: Seq[Int]) =>
  ...
  def getVolumesPlatformCountry(
    ...
    val df = data
      .groupBy("platform", "segment", "country")
      .count()
    df
  }
  ...
  **/
  ...
  def getReportPlatformsCountry(
    ...
    saveData(df = df_temp, path = temp_path)
    ...
    saveData(df = df1, path = path_report)
    ...
    saveData(df = df2, path = path_report_country)
