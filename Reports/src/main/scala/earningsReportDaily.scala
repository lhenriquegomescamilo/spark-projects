...
//////////////////////////////////////////////////////////////
  ...
  def saveRelevantDevicesDF(
    ...
    val df_nDays =
      getDataTriplets(spark, nDays, since).filter("id_partner NOT IN (1, 119)")
    ...
    saveData(data = df, path = "/datascience/reports/earnings/temp/")
  ...
  def getJoint_xd(
    ...
    val udfMap_countries = udf(
      (segments: Seq[Int]) => (segments flatMap (seg_country_map get)).lift(0)
    )
    ...
    val xd_map = ((mapping1
      .union(mapping2))
      .collect()
      .map(row => (row(1).toString.toInt, row(0).toString.toInt))
      .toList)
      .toMap

    /** udf to map xd segments to their parent segment */
    val udfMap_segs = udf((segments: Seq[Int]) => segments flatMap (xd_map get))
  ...
  def getCountbyCountry(
    ...
    val window = Window
      .partitionBy(col("segment"), col("device_id"), col("country"))
    ...
    val data = getGroupedbyCountry(dfy = dfy, date_current = date_current)
  ...
  **/
  ...
  def getDataReport_xd(
    ...
    val df_grouped_country =
      getGroupedbyCountry(dfy = df_xd, date_current = date_current)
  ...
  /**
    ...
    * @param since: number of days since to query.
    *
    * As a result this method stores files in /datascience/reports/earnings/ partitioned by day
  **/
  def getDataReport(spark: SparkSession, nDays: Integer, since: Integer) = {
    ...
    saveRelevantDevicesDF(spark = spark, nDays = nDays, since = since)
    ...
    saveData(data = df_count_country, path = savepath1)
  ...
  def main(Args: Array[String]) {
    ...
    getDataReport(spark = spark, nDays = nDays, since = since)
